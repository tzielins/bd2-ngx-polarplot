...
export class PolarDomainUtil {
  ...
  dataToPetal(data: number[], domain: number[],
    ...
    let node = this.calculatePetalProperties(data, domain, scaleRadius, scaleWidth, error);
  ...
  calculateCircularMeanAndDev(data: number[], domain: number[]) {
    ...
    let m = d3.mean(data);
    let std = d3.deviation(data) || 0;

    let rot = domain[2] / 2;
    data = data.map(v => v < rot ? v + domain[2] : v);
    let rotM = d3.mean(data) % domain[2];
    let rotStd = d3.deviation(data) || 0;
  ...
  calculatePetalProperties(data: number[], domain: number[],
    ...
    let node = new PetalNode();

    let meanStd = this.calculateCircularMeanAndDev(data, domain);
